...
- 当前真实通知域已落地并直接消费 `outcome_summary` 与 `issue_count`；其中失败作业若已对应到活动异常，则通知投影层不会再额外生成失败作业提醒。
...
截至 2026-04-12，这套任务域表已经不是纯设计稿：

- `jobs`、`job_items`、`job_events` 已经成为任务中心的真实数据来源
- 客户端通过 `TaskCenterWorkspace` 和 `jobsApi` 拉取列表、详情、事件与控制结果
- 通知域已开始真实消费 `jobs` 的终态、`issue_count` 与 `outcome_summary`，用于生成提醒类通知并抑制与活动异常重复的失败提醒
- `/api/events/stream` 已用于任务状态实时刷新
- 文件中心切片 7 已开始稳定消费这套模型：`REPLICATE`、`DELETE_REPLICA`、`DELETE_ASSET` 都已进入真实作业链路，并通过 `job_object_links` 回连资产、副本、目录、挂载
