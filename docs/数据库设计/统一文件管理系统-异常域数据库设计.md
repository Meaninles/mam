...
- 当前状态：已在中心服务中完成第一版真实落地；本文仍保留为异常域数据库设计口径说明，但应以仓库中的 `services/center/migrations/0007_issues.sql` 与 `services/center/internal/issues/` 为准
...
| `occurrence_count` | `integer` | 是 | `1` | 同一异常对象累计发生次数 |
| `last_detection_key` | `text` | 否 | `null` | 最近一次检测指纹集合快照，用于避免同一轮失败重复累加次数 |
| `title` | `text` | 是 | 无 | 列表标题 |
...
- 当前真实实现已把 `dedupe_key` 用作服务层复用入口，按“标题 / 类型 / 分类 / 来源域 / 关键来源上下文 / 错误摘要”等稳定组合键在历史异常中复用同一治理对象，支持 `A -> B -> A` 回并到第一个 `A`。
- `occurrence_count` 与 `last_detection_key` 已在真实 migration 中落地，用于累计同一异常的发生次数，并避免同一轮检测或同一条失败事实被重复计数。
...
补充说明：当前真实 migration 已落地，但出于当前产品收口考虑，已删除“延后处理”“刷新检测”两条正式治理动作；因此实际可见状态与事件枚举较本文第一稿更收敛。
...
补充说明：当前仓库中通知域已经落地第一版真实实现，并直接复用异常对象作为异常通知来源，因此异常去重策略会直接影响通知是否重复提醒。
