...
- 文档目标：记录当前仓库已经落地的存储节点、挂载关系、节点凭据、节点运行态、挂载运行态数据库口径，并作为后续 migration、Service 与文档同步的基准
- 当前状态：已落地到 `services/center/migrations/0002_storage_local_folders.sql`，并由 `services/center/internal/storage/*` 提供配套服务实现与测试
...
- 主数据库为 PostgreSQL。
- 当前已落地 migration 中，主键、外键和业务标识统一采用 `text`，时间使用 `timestamptz`。
- 枚举型字段第一版优先采用 `text`；部分字段配 `check constraint`，部分字段暂由服务层约束。
- 敏感凭据不得直接与主配置表混放。
...
- 主键：`storage_nodes_pkey (id)`
- 唯一约束：`storage_nodes_code_key (code)`
- 检查约束：
  - `node_type in ('LOCAL', 'NAS', 'CLOUD')`
- 现实说明：
  - 当前 migration 没有对 `access_mode` 添加检查约束
  - `code`、`name` 依赖 `NOT NULL` 与服务层校验，不依赖空字符串检查
...
- 当前实现口径里，`LOCAL` 使用 `DIRECT`，`NAS` 使用 `SMB`，`CLOUD` 使用 `QR`。
...
| --- | --- | --- | --- | --- |
| `id` | `text` | 是 | 无 | 主键 |
| `storage_node_id` | `text` | 是 | 无 | 所属存储节点 |
...
- 检查约束：
  - 当前 migration 未对 `credential_kind`、`token_status` 添加 `check constraint`
- 现实说明：
  - `credential_kind` 当前实际使用值包括 `NONE`、`USERNAME_PASSWORD`、`TOKEN`
  - `token_status` 当前实际使用值包括 `UNKNOWN`、`CONFIGURED`、`PENDING_SCAN`
...
| `library_id` | `text` | 是 | 无 | 所属资产库 |
| `library_name` | `text` | 是 | 无 | 所属资产库名称 |
| `storage_node_id` | `text` | 是 | 无 | 对应存储节点 |
| `name` | `text` | 是 | 无 | 挂载名称 |
...
- 主键：`pk_mounts (id)`
- 外键：
  - `mounts_storage_node_id_fkey -> storage_nodes(id)`
- 唯一约束：
  - `mounts_code_key (code)`
- 检查约束：
...
- 现实说明：
  - 当前 migration 尚未对 `library_id` 建外键
  - 当前 migration 尚未增加 `(library_id, storage_node_id, source_path)` 联合唯一约束
  - `name`、`source_path`、`relative_root_path` 主要依赖 `NOT NULL` 与服务层校验
...
| `auth_status` | `text` | 是 | `'NOT_REQUIRED'` | 挂载级鉴权状态 |
...
### 5.5.4 设计说明
 ...
 - 扫描历史已经单独落到 `mount_scan_histories`。

## 5.6 `mount_scan_histories`
...
| `id` | `text` | 是 | 无 | 主键 |
| `mount_id` | `text` | 是 | 无 | 所属挂载 |
| `started_at` | `timestamptz` | 是 | 无 | 开始时间 |
| `finished_at` | `timestamptz` | 是 | 无 | 结束时间 |
| `status` | `text` | 是 | 无 | 扫描结果 |
| `summary` | `text` | 是 | 无 | 摘要 |
| `trigger` | `text` | 是 | 无 | 触发方式 |
| `created_at` | `timestamptz` | 是 | `now()` | 创建时间 |

### 5.6.2 约束设计

- 主键：`pk_mount_scan_histories (id)`
- 外键：`fk_mount_scan_histories_mount_id -> mounts(id)`
- 检查约束：`status in ('SUCCESS', 'FAILED')`

### 5.6.3 设计说明

- 扫描历史用于挂载级追溯、页面展示和异常联动。
- 该表已经在第一版 migration 中直接落地，而不是后续预留。
...
10. 第一版优先使用 `text` 字段，按需要补 `check constraint`，不急于引入 PostgreSQL enum。
11. `storage_node_credentials` 保留 `secret_ciphertext` 和 `secret_ref` 双模式。
12. 扫描历史不进入主运行态表，而是已经独立落到 `mount_scan_histories`。
...
- 凭据、运行态、静态配置全部混在主表中
- 后续一旦接入真实扫描、鉴权、网盘二维码状态回写，很快还会再拆表
- 不利于安全边界和运行态隔离
...
如果本稿通过，可以把这 6 张表视为当前存储域数据库设计的代码对齐稿。

后续最自然的下一步有两个：

1. 基于本稿继续整理增量 migration 设计，而不是重新设计已落地核心表。
2. 继续补与现有存储域表强相关的外围表：
   - `jobs`
   - `issues`
   - `worker_registrations`

当前最推荐先进入 `jobs` / `issues` / `worker_registrations` 设计，因为 `mount_scan_histories` 已经随切片 1 migration 落地。
