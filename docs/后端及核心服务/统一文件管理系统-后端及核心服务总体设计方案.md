...
说明：存储域中的本地文件夹、NAS、网盘核心闭环已经先行落地在 `services/center/internal/storage/` 与对应 migration 中；资产域当前已在 `services/center/internal/assets/` 中落地 browse / detail / create directory / upload / scan / annotations / delete entry 等核心能力。当前仍需要继续收口的是标签、作业化重操作、客户端调用边界与本地执行器能力。
