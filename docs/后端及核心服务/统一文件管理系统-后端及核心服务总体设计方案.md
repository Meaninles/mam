...
- 客户端对存储节点中心服务接口的完全收口
- 本地执行器侧的存储节点业务模块
- 导入深度预检、强校验与 NAS/CLOUD 目标导入执行

说明：存储域中的本地文件夹、NAS、网盘核心闭环已经先行落地在 `services/center/internal/storage/` 与对应 migration 中；资产域当前已在 `services/center/internal/assets/` 中落地 browse / detail / create directory / upload / scan / annotations / delete entry 等核心能力。作业域当前已在 `services/center/internal/jobs/` 中落地统一作业模型、调度器、查询 / 控制接口和作业 SSE 基础能力。异常域当前已在 `services/center/internal/issues/` 中落地核心表、服务、查询 / 动作接口，以及与真实作业状态同步的闭环。导入域当前已在 `services/center/internal/importing/` 中落地真实导入会话、导入草稿/计划、目录级懒加载浏览、真实 `IMPORT` 作业接线、正式导入报告与最基本的轻校验。当前仍需要继续收口的是云端导入执行、导入深度预检、强校验以及客户端部分旧测试。
...
- 客户端设置页、文件中心、任务中心、异常中心已经消费这套真实状态：设置页展示依赖服务运行态，文件中心按运行态和鉴权状态禁用云端动作，任务中心展示 `CD2_REMOTE_UPLOAD` 与 `ARIA2` 的阶段文案
