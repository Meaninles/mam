...
说明：存储域中的本地文件夹、NAS、网盘核心闭环已经先行落地在 `services/center/internal/storage/` 与对应 migration 中；资产域当前已在 `services/center/internal/assets/` 中落地 browse / detail / create directory / upload / scan / annotations / delete entry 等核心能力。作业域当前已在 `services/center/internal/jobs/` 中落地统一作业模型、调度器、查询 / 控制接口和作业 SSE 基础能力。异常域当前已在 `services/center/internal/issues/` 中落地核心表、服务、查询 / 动作接口，以及与真实作业状态同步的闭环。当前仍需要继续收口的是通知、导入、客户端部分旧测试与本地执行器后续能力。
