...
- 设计原则：不被当前前端原型编排方式绑定，但充分复用现有客户端中已经验证过的业务对象、交互闭环、状态机与命名语义
...
截至 2026-04-14，仓库中已经落地了切片 0 所需的最小三段式底座，存储节点、任务、异常、通知、导入以及 115 云传输链路都已进入真实实现阶段。
...
- 资产域 read model migration（`0003_assets_read_model.sql`）
- 任务域 migration（`0006_jobs.sql`）
- 开发态 Docker PostgreSQL 编排
- 客户端顶层壳层系统状态灯接线
- 存储节点真实服务、路由、数据库 migration 与配套测试
- 资产库 / 目录树 / 文件条目浏览、详情、扫描、上传、标记更新、删除的真实服务与配套测试
- 作业域真实服务、调度、事件写入、基础 SSE 推送与配套测试
- 扫描作业真实接线：文件中心目录扫描、存储节点挂载扫描均已改为“创建作业 -> 中心服务调度执行”
- 导入域真实模型、迁移、接口与配套测试
- 本地执行器真实导入能力：Windows 外接 USB 设备发现、按层级浏览、本地复制、轻校验
- 导入中心真实接线：按层级懒加载浏览、目录/文件选择、真实 `IMPORT` 作业、正式导入报告、轻校验统计
- `CloudDrive2 + aria2 + 115` 集成
- `services/center/internal/integration/provider_cd2_115.go` 中的 115 Token/扫码鉴权、远程上传、下载地址解析、云端删除
- `services/center/internal/integration/downloader_aria2.go` 中的 aria2 二进制准备、RPC 拉起、下载入队、恢复、暂停、继续、取消与运行态检测
- `services/center/internal/assets/heavy_ops_cloud.go` 中的 `LOCAL/NAS -> CLOUD` 上传、`CLOUD -> LOCAL/NAS` 下载、云端副本回写、外部任务状态恢复与取消协同
- 设置页“依赖服务”子页、文件中心云端动作可用性判断、任务中心外部任务阶段映射

当前未落地部分：

- 非作业域中尚未落地的统一事件模型（如资产变化等统一 SSE 事件）
- `CLOUD -> CLOUD` 同步、离线下载 + 自动转存等云端增强链路
- 历史数据清理后的开发环境重建与后续生产化策略
- 客户端对存储节点中心服务接口的完全收口
- 本地执行器侧的存储节点业务模块
- 导入深度预检、强校验，以及导入结果的进一步验收与结果收口

说明：存储域中的本地文件夹、NAS、网盘核心闭环已经先行落地在 `services/center/internal/storage/` 与对应 migration 中；资产域当前已在 `services/center/internal/assets/` 中落地 browse / detail / create directory / upload / scan / annotations / delete entry 等核心能力。作业域当前已在 `services/center/internal/jobs/` 中落地统一作业模型、调度器、查询 / 控制接口和作业 SSE 基础能力。异常域当前已在 `services/center/internal/issues/` 中落地核心表、服务、查询 / 动作接口，以及与真实作业状态同步的闭环。导入域当前已在 `services/center/internal/importing/` 中落地真实导入会话、导入草稿/计划、目录级懒加载浏览、真实 `IMPORT` 作业接线、正式导入报告、最基本的轻校验，以及 `LOCAL` / `NAS` 目标端的查询、选择与执行链路。当前仍需要继续收口的是导入结果的跨页回写验收、导入深度预检、强校验以及客户端部分旧测试。
...
- 浏览阶段已改成按层级懒加载，不再在打开导入中心时全量展开整盘文件
- 导入对象通过 `import_plan_items` 保存，提交导入后再由后台任务递归展开目录树
- 当前默认启用最基本的轻校验（大小 + 修改时间），强校验仅预留模型与后续实现口子
...
- 当前真实实现已补充：通知源数据由 `notifications` 表承接，通知以异常对象和作业终态为来源投影生成；对于已存在活动异常的失败作业，不再额外生成失败作业提醒
...
其中早期文档里的 `entry_tag_links` 已演进为两张 link 表：

- `asset_tag_links`：资产与标签关联
- `directory_tag_links`：目录与标签关联
...
- `/api/integrations/gateways/cd2`
- `/api/integrations/gateways/cd2/test`
- `/api/integrations/runtime`

其中截至 2026-04-11 已真实落地的资产相关接口为：
...
其中截至 2026-04-11 已真实落地的标签相关接口为：

- `GET /api/tags/management`
- `GET /api/tags/suggestions`
- `POST /api/tags/groups`
- `PATCH /api/tags/groups/{id}`
- `POST /api/tags/groups/{id}/move`
- `POST /api/tags`
- `PATCH /api/tags/{id}`
...
当前落地补记（截至 2026-04-12）：

- 该分段的读路径已经落地，客户端文件中心主读取链路已切到中心服务
- 一部分轻写入也已提前落地，包括新建目录、扫描、上传、文件级星级 / 色标更新、文件 / 目录标签保存与批量标签增量应用，以及条目删除
- 标签域已落地到中心服务并与文件中心共用同一套 PostgreSQL 数据
- 当前实现已移除备注 / 基础元数据编辑的正式接口职责与客户端 UI 语义，不再将其视为切片 6 正式能力
- 其中“扫描”已经在切片 4 中进入统一作业体系；截至 2026-04-12，切片 7 对应的端点级同步 / 删除副本 / 删除资产也已进入统一作业体系
...
- 作业域核心已落地：`0006_jobs.sql`、`shared/contracts/dto/job`、`services/center/internal/jobs/`
- 已落地统一作业查询 / 控制接口与 `/api/events/stream` 作业 SSE 基础能力
- 已接入的真实业务载体已扩展为：文件中心目录扫描、端点同步、删除端点副本、删除资产、存储节点挂载扫描
- 文件中心已新增 `POST /api/file-entries/replicate`、`POST /api/file-entries/delete-replicas`、`POST /api/file-entries/delete-assets`
- 删除资产真实作业已支持批量删除时同时处理所选目录：先删资产，再按目录深度倒序删目录
- 当前真实执行范围已覆盖 `LOCAL` / `NAS` / `CLOUD`：`LOCAL/NAS -> CLOUD` 走 CloudDrive2 远程上传，`CLOUD -> LOCAL/NAS` 由 CloudDrive2 解析下载地址后交给 aria2 下载；`CLOUD -> CLOUD` 仍未支持
- 文件中心动作当前会同时校验 `CloudDrive2` 运行态、`aria2` 运行态与 115 节点鉴权状态，不满足条件时会在页面侧禁用动作并给出跳转入口
- 任务中心 UI 已改接真实作业消费层；异常域与通知域均已落地
...
- 当前已额外落地：按层级懒加载浏览、目录/文件选择模型、多目标稳定导入、最基本轻校验与正式导入报告统计
...
当前落地补记（截至 2026-04-14）：

- `services/center/internal/integration/service.go` 已支持外部任务的暂停、继续、取消，并向作业事件流回写 `externalTaskEngine / externalTaskId / externalTaskStatus`
- `provider_cd2_115.go` 已支持 115 Token 鉴权、115 Open OAuth、二维码登录、远程上传、下载地址解析、云端删除与上传状态轮询
- `downloader_aria2.go` 已支持自动准备 Windows x64 aria2、启动本地 JSON-RPC 服务、通过 `aria2.session` 持久化会话，并在恢复时复用既有下载或按目标路径找回任务
- `heavy_ops_cloud.go` 已把云上传、云下载与云端删副本接入统一作业体系，完成后会把真实副本写回 `asset_replicas`
- 客户端设置页、文件中心、任务中心、异常中心、通知中心已经消费这套真实状态：设置页展示依赖服务运行态、最近错误代码与凭据状态，文件中心按运行态和鉴权状态禁用云端动作，任务中心展示 `CD2_REMOTE_UPLOAD` 与 `ARIA2` 的阶段文案，异常中心/通知中心按云端问题分类提供设置页或存储节点页跳转
- 当前仍未完成的边界集中在 `CLOUD -> CLOUD`、离线下载 + 自动转存，以及更强的外部状态持久化与恢复治理
...
- 5 已完成作业域所需最小事件契约与基础 SSE 推送能力，但尚未扩展到异常 / 通知等跨域事件
- 6 尚未正式纳入当前仓库的一键开发编排；当前开发联调仍以手工准备 CloudDrive2 与项目内 aria2 为主
...
其中存储节点相关迁移与服务已经先行落地，当前更重要的是把客户端、执行器和其余业务域继续收口到统一后端。
...
- 13、14 已完成第一版真实实现，并已用于扫描作业
- 15 已完成首版真实实现，并已接通异常治理动作与跨页跳转
- 16 已完成首版通知投影、通知查询与通知 SSE
...
- [x] 中心服务成为文件中心权威数据源，客户端已移除本地 `sql.js + localStorage` 文件中心 fallback
- [ ] 文件中心能基于真实目录树与副本状态展示资产
- [x] 任务中心能展示真实作业主任务、子项和状态流转
- [x] 异常中心能展示由扫描、传输、删除等真实作业产生的异常，并支持基础治理动作与跨页跳转
- [ ] 导入中心能通过本地执行器完成真实设备会话、导入计划与导入执行
- [x] 115 上传下载可通过 CloudDrive2 + aria2 真实跑通
- [ ] 删除确认链路能正确识别哪些副本删除会导致资产删除
- [x] 通知中心已能基于中心服务事件生成通知，消费状态仅保留在当前客户端
