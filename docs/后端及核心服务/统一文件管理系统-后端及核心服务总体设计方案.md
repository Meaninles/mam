...
| --- | --- | --- |
| `id` | uuid | 主键 |
| `library_id` | uuid | 所属资产库 |
| `relative_path` | text | 相对资产库根路径，根目录为 `/` |
| `name` | text | 目录名 |
| `parent_path` | text | 父目录相对路径 |
| `source_kind` | text | `SCANNED` / `MANUAL` / `DERIVED` |
| `presence_status` | text | 目录可见状态 |
| `created_at` | timestamptz | 创建时间 |
| `updated_at` | timestamptz | 更新时间 |

唯一约束：`(library_id, relative_path)`
...
| `name` | text | 文件名 |
| `extension` | text | 扩展名 |
| `size_bytes` | bigint | 当前资产标准大小 |
| `mime_type` | text | 推断类型 |
| `lifecycle_state` | text | `ACTIVE` / `DELETE_PENDING` / `DELETED` |
| `rating` | int | 星级 |
| `color_label` | text | 色标 |
| `note` | text | 备注 |
| `created_at` | timestamptz | 创建时间 |
...
建议唯一约束：`(library_id, relative_path)`
...
| `asset_id` | uuid | 资产 ID |
| `mount_id` | uuid | 所属挂载 |
| `storage_node_id` | uuid | 存储节点 ID |
| `physical_path` | text | 节点上的实际路径 |
| `relative_path` | text | 在资产库中的相对路径 |
| `size_bytes` | bigint | 副本大小 |
| `modified_at` | timestamptz nullable | 副本修改时间 |
| `replica_state` | text | `AVAILABLE` / `MISSING` / `SYNCING` / `VERIFYING` / `ERROR` / `DELETE_PENDING` |
| `sync_state` | text | `IN_SYNC` / `OUT_OF_SYNC` / `UNKNOWN` |
| `quick_hash` | text nullable | 轻量校验值 |
| `hash_verified_at` | timestamptz nullable | 最近强校验时间 |
| `last_seen_at` | timestamptz | 最近扫描看到时间 |
| `missing_detected_at` | timestamptz nullable | 最近一次发现丢失时间 |
| `created_at` | timestamptz | 创建时间 |
| `updated_at` | timestamptz | 更新时间 |

唯一约束建议：`(mount_id, relative_path)`
...
| --- | --- | --- |
| `id` | uuid | 主键 |
| `directory_id` | uuid | 目录节点 ID |
| `mount_id` | uuid | 挂载 ID |
| `physical_path` | text | 实际目录路径 |
| `presence_state` | text | `PRESENT` / `MISSING` / `UNKNOWN` |
| `last_seen_at` | timestamptz | 最近看到时间 |
| `updated_at` | timestamptz | 更新时间 |
...
| --- | --- | --- |
| `id` | uuid | 主键 |
| `node_type` | text | `LOCAL` / `NAS` / `CLOUD_115` |
| `driver` | text | `LOCAL_FS` / `SMB` / `CD2_115` |
| `name` | text | 节点名称 |
| `status` | text | `ONLINE` / `OFFLINE` / `DEGRADED` |
| `writable` | boolean | 是否可写 |
| `capabilities` | jsonb | 支持能力清单 |
| `config_json` | jsonb | 节点接入配置 |
| `last_heartbeat_at` | timestamptz nullable | 最近心跳 |
| `last_check_at` | timestamptz nullable | 最近连接测试 |
| `created_at` | timestamptz | 创建时间 |
| `updated_at` | timestamptz | 更新时间 |
...
| --- | --- | --- |
| `id` | uuid | 主键 |
| `library_id` | uuid | 资产库 ID |
| `storage_node_id` | uuid | 存储节点 ID |
| `mount_root_path` | text | 节点上对应资产库根的路径 |
| `enabled` | boolean | 是否启用 |
| `writable` | boolean | 是否可写 |
| `scan_policy` | jsonb | 扫描策略 |
| `verify_policy` | jsonb | 校验策略 |
| `sort_order` | int | 展示顺序 |
| `created_at` | timestamptz | 创建时间 |
| `updated_at` | timestamptz | 更新时间 |

约束建议：一个节点可以挂入多个资产库，但同一库内同一路径挂载必须唯一。
...
| `id` | uuid | 主键 |
| `mount_id` | uuid | 挂载 ID |
| `trigger_type` | text | `MANUAL` / `SCHEDULED` / `AUTO` |
| `status` | text | `PENDING` / `RUNNING` / `SUCCEEDED` / `FAILED` / `PARTIAL_SUCCESS` |
| `started_at` | timestamptz | 开始时间 |
| `finished_at` | timestamptz nullable | 结束时间 |
| `summary_json` | jsonb | 扫描摘要 |
...
| --- | --- | --- |
| `id` | uuid | 主键 |
| `job_type` | text | `SCAN` / `IMPORT` / `SYNC_COPY` / `SYNC_UPLOAD_115` / `SYNC_DOWNLOAD_115` / `DELETE_REPLICA` / `DELETE_ASSET` / `VERIFY` / `CLEANUP` |
| `status` | text | 主状态 |
| `priority` | text | 优先级 |
| `library_id` | uuid nullable | 所属资产库 |
| `source_ref` | text nullable | 来源描述 |
| `target_ref` | text nullable | 目标描述 |
| `created_by` | text | 来源模块 |
| `executor_scope` | text | `CENTER` / `AGENT` / `CD2` / `ARIA2` / `MIXED` |
| `retry_limit` | int | 最大重试次数 |
| `retry_count` | int | 当前重试次数 |
| `progress_percent` | numeric | 当前进度 |
| `status_message` | text | 状态摘要 |
| `context_json` | jsonb | 作业上下文 |
| `created_at` | timestamptz | 创建时间 |
| `updated_at` | timestamptz | 更新时间 |
...
| `id` | uuid | 主键 |
| `job_id` | uuid | 作业 ID |
| `asset_id` | uuid nullable | 关联资产 |
| `replica_id` | uuid nullable | 关联副本 |
| `item_type` | text | `FILE` / `DIRECTORY` / `STEP` |
| `status` | text | 子项状态 |
| `progress_percent` | numeric | 子项进度 |
| `result_summary` | text nullable | 结果摘要 |
| `context_json` | jsonb | 子项上下文 |
| `created_at` | timestamptz | 创建时间 |
| `updated_at` | timestamptz | 更新时间 |
...
记录状态迁移、异常、进度回写、重试、取消等事件，作为后续任务中心和审计的统一来源。
...
| --- | --- | --- |
| `id` | uuid | 主键 |
| `worker_type` | text | 当前为 `LOCAL_AGENT` |
| `machine_name` | text | 机器名 |
| `host` | text | 地址 |
...
| `category` | text | 一级分类 |
| `severity` | text | 严重级别 |
| `status` | text | `OPEN` / `CONFIRMED` / `POSTPONED` / `IGNORED` / `RESOLVED` / `ARCHIVED` |
| `source_domain` | text | 来源域 |
| `library_id` | uuid nullable | 所属资产库 |
| `asset_id` | uuid nullable | 关联资产 |
| `replica_id` | uuid nullable | 关联副本 |
| `job_id` | uuid nullable | 关联作业 |
| `summary` | text | 摘要 |
| `detail` | text | 详情 |
| `capabilities` | jsonb | 支持的治理动作 |
| `created_at` | timestamptz | 创建时间 |
| `updated_at` | timestamptz | 更新时间 |
...
8. `storage_nodes` / `library_mounts` 迁移与读写
9. `assets` / `asset_metadata` / `asset_replicas` 迁移与读写
10. 目录树合并查询服务
11. 默认资产判定服务
12. 标签域迁移与服务化
...
13. `jobs` / `job_items` / `job_events` 迁移与仓储
14. 作业状态机与调度器
15. `issues` / `issue_events` 迁移与治理动作
16. 面向客户端的通知事件输出
