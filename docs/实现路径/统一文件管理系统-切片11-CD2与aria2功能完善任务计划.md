...
- 当前剩余工作不再是页面缺失，而是 `CLOUD -> CLOUD`、离线下载 + 自动转存、云端导入端到端验收等后续边界。
...
- 异常中心与通知中心已能按云端问题类型执行跳转治理：
  - [IssuesPage.tsx](/B:/new_project/mare/client/src/pages/IssuesPage.tsx)
  - [NotificationCenterSheet.tsx](/B:/new_project/mare/client/src/pages/NotificationCenterSheet.tsx)
- 导入中心已正式暴露 `CLOUD` 目标端，并进入中心服务编排查询链路：
  - [ImportCenterPage.tsx](/B:/new_project/mare/client/src/pages/ImportCenterPage.tsx)
...
- 已正式开放 `CLOUD` 目标端导入编排。
- 已明确并回写文档：`CLOUD` 目标已经进入可查询目标端、资产库目标过滤与按 ID 回查链路。
- 已刷新导入中心文档中“NAS / CLOUD 目标正式导入执行未完成”的旧口径，改为“已开放 CLOUD 编排入口，但端到端验收仍待继续补齐”。
- 当前剩余边界：提交导入后的云端结果在任务中心、文件中心、异常中心、通知中心四页中的端到端一致性验收仍需后续继续收口。
