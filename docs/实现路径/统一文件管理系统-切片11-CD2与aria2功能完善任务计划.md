...
- 页面已有 `CloudDrive2` 集成卡片
- App 已能同时加载 `gateways` 与 `runtime`
- 但前端当前只消费 `CloudDrive2`，没有把 `aria2` 一并作为正式运行组件展示

### 4.2 当前还差哪些工作

- 补齐 `CloudDrive2 + aria2` 双组件运行态展示，而不是只显示 `CloudDrive2`
- 展示最近错误信息、最近检测时间、是否已有凭据等关键运维信息
- 统一保存成功、测试成功、测试失败后的提示语义
- 统一“禁用集成”和“连接异常”的页面视觉区分
- 为后续页面提供可消费的全局集成健康状态，而不是让每页自行猜测
...
- 文件中心的重操作入口已经都走正式接口
- 后端已经支持 `UPLOAD / DOWNLOAD / CLOUD DELETE`
- 但前端动作判断仍主要依赖端点状态，没有纳入集成运行态
- 页面文档仍保留“`CLOUD` 端点仅保留计划编排入口”的旧口径

### 6.2 当前还差哪些工作

- 把 `115` 端点动作从“仅凭状态可点”升级为“结合集成运行态判断是否可执行”
- 补齐云端上传、下载、删除副本的动作前置提示
- 明确 `UPLOAD / DOWNLOAD / COPY` 的页面语义，不再把云端动作混在普通复制中
- 动作失败时给出更具体的指向，例如跳转到设置页或存储节点页
- 刷新文件中心文档口径，移除“云端执行未接入”的陈旧表述
...
- 在任务详情或子项详情中展示外部执行器信息
- 展示外部任务 ID、外部状态、原始错误信息
- 区分“中心服务状态失败”和“外部执行器失败”
- 补齐云端任务的可观测性字段，方便定位是否是 `CD2`、`aria2`、115 或目标路径问题
