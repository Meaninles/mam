...
- 本文所列 7 个页面收口任务已按顺序完成首轮实现回写。
- 已补齐自动化测试，覆盖设置页、存储节点页、异常中心、通知中心、导入中心，以及相关共享契约/服务端行为。
- 已新增独立真实客户端测试文档：[统一文件管理系统-切片11页面收口真实客户端测试用例.md](/B:/new_project/mare/docs/实现路径/统一文件管理系统-切片11页面收口真实客户端测试用例.md)。
- 当前剩余工作不再是页面缺失，而是 `CLOUD -> CLOUD`、离线下载 + 自动转存等后续边界；导入侧不再规划导入到 `CLOUD`。
...
- 设置页已能显示 CloudDrive2 / aria2 依赖服务卡片，并执行保存、测试、运行状态展示，同时展示最近错误代码与凭据状态：
...
- 存储节点页已能保存网盘节点、创建二维码会话、轮询二维码状态、运行网盘连接测试，并展示云节点治理字段：
...
- 异常中心与通知中心已能按云端问题类型执行跳转治理：
  - [IssuesPage.tsx](/B:/new_project/mare/client/src/pages/IssuesPage.tsx)
  - [NotificationCenterSheet.tsx](/B:/new_project/mare/client/src/pages/NotificationCenterSheet.tsx)
- 导入中心正式目标端已收敛为 `LOCAL` / `NAS`，不再暴露 `CLOUD` 导入目标：
  - [ImportCenterPage.tsx](/B:/new_project/mare/client/src/pages/ImportCenterPage.tsx)
  - [services/center/internal/importing/service.go](/B:/new_project/mare/services/center/internal/importing/service.go)
- App 已在启动时加载集成网关与运行态：
...
本轮页面收口完成后，仍需继续推进的边界主要集中在：

- `CLOUD -> CLOUD` 同步仍未支持
- 离线下载 + 自动转存仍未支持
- 云端导入结果在任务中心、文件中心、异常中心、通知中心四页的端到端验收仍待补齐
- 更强的外部任务状态持久化、恢复诊断与运维治理仍待后续继续增强
...
- CloudDrive2 支持保存与连接测试，aria2 由中心服务托管展示运行状态、错误代码与错误摘要

### 4.2 当前已完成收口与剩余边界

- 依赖服务页已具备双组件展示与运行状态汇总。
- 已展示最近检测时间、错误摘要、`lastErrorCode`、是否已保存凭据等运维信息。
- 集成健康状态已由 App 汇总并提供给文件中心动作门禁。
- 异常中心与通知中心中的 `CloudDrive2/aria2` 问题已可直接跳到设置页处理。
- 当前剩余边界：未提供“禁用集成”的独立开关，也未将更广泛的系统依赖纳入同一工作台。
...
- 已支持 `Token` 和扫码登录两种入口
- 已支持二维码创建、轮询、连接测试
- 当前列表与编辑弹层已经可以承载首轮生产排障，但更细粒度的历史审计仍待后续补充

### 5.2 当前已完成收口与剩余边界

- 列表中已补齐 `Token 状态`、账号别名、最近鉴权结果、最近错误代码与最近失败原因等治理字段。
- 编辑弹层中已补齐“当前是否已有有效凭据”的摘要反馈。
- 批量连接测试与单项连接测试已经统一映射到更清晰的用户文案。
- 相关旧文档里“没有真实 NAS / 网盘鉴权全链路完全收口”的口径已开始收口。
- 当前剩余边界：更多历史鉴权轨迹与更细粒度的凭据审计仍未进入客户端工作台。
...
- 文件中心已经按 `cd2Online / aria2Online / cloudAuthReady` 共同控制云端动作可用性。
- 云端动作受限时，页面会明确说明是 `CloudDrive2`、`aria2` 还是 115 鉴权导致不可执行，并提供跳转入口。
- 当前剩余边界：`CLOUD -> CLOUD` 同步与离线下载 + 自动转存仍未支持。
...
- 当前剩余边界：更强的外部状态持久化、恢复诊断与长链路观测仍待后续继续增强。
...
- 当前已具备云端问题分类、治理入口与跨页跳转，但仍需为后续新执行器预留扩展位

### 8.2 当前已完成收口与剩余边界

- 云端传输问题已明确分为 `115 鉴权问题`、`CloudDrive2 网关问题`、`aria2 下载器问题`、`云端路径问题`、`传输执行问题`。
- 异常详情中已补齐云端上下文，并展示云端问题类型。
- 已明确从异常中心跳设置页、存储节点页或任务中心的优先路径，其中 `CloudDrive2/aria2` 直达设置页。
- 当前剩余边界：若后续增加新的云端执行器，分类规则仍需继续扩展。
...
- 云端运行态问题已纳入正式通知来源。
- 通知中心已按同一套云端问题分类规则区分处理型异常与运行态提醒。
- 已为 `CD2 离线 / aria2 未就绪 / 115 鉴权失效 / 云端目录校验失败` 提供准确跳转。
- 当前剩余边界：通知去重与更细粒度的抖动抑制仍可继续增强。
...
- 已移除 `CLOUD` 目标端导入编排，导入目标正式收敛为 `LOCAL` / `NAS`。
- 已明确并回写文档：导入中心不再把 `115网盘` 作为导入目标端。
- 已刷新导入中心文档中旧的 `CLOUD` 导入描述，避免继续把网盘写成导入目标。
- 当前剩余边界：提交导入后的结果在任务中心、文件中心、异常中心、通知中心四页中的端到端一致性验收仍需后续继续收口。
...
1. 让导入中心继续只暴露真实可执行的 `LOCAL` / `NAS` 目标端
2. 收口提交前校验与目标端可用性判断
3. 收口提交后云端结果的跨页回写
4. 刷新导入中心页面文档
...
虽然本计划按页面组织，但切片 11 进入生产环境前，仍需持续推进以下跨页收尾：

- 相关页面设计文档已开始回写并持续消除旧口径
- 与页面相匹配的自动化测试已补齐，并已新增独立真实客户端测试用例文档
- 至少完成一条真实烟测路径：
  - 配置 CD2
  - 配置 115 节点
  - 从文件中心发起上传到 115
...
切片 11 当前缺的已经不再是“后端能不能接 CD2 / aria2”，本轮首要页面收口也已完成；剩下的重点是继续把云端增强链路、导入端到端验收与更强的运维恢复治理打磨成生产级体验。
