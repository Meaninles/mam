...
func (dpapiCredentialCipher) Encrypt(plaintext string) (string, error) {
	if plaintext == "" {
		return "", nil
	}

	input := bytesToDataBlob([]byte(plaintext))
	...
	return base64.StdEncoding.EncodeToString(dataBlobBytes(&output)), nil
}
